...
- fixed/guarded in semantic-helper scrub: generic inference shape item matching, resolver scoped type-reference validation, executable/test target single-source lowering, C match branch body emission, and behavior-bound generated-C assertion ladders now share focused helpers without weakening file-focus hygiene tests.

...
- Generic `Vec<T>` struct/method specialization is now pinned in typed, HIR,
  and MIR JSON by `emit_json_typed_generic_vec_schema_matches_golden`,
  `emit_json_hir_generic_vec_schema_matches_golden` and
...
- Generic worklist deduplication is now pinned in typed, HIR, and MIR JSON by
  `emit_json_typed_generic_worklist_dedup_schema_matches_golden`,
...
- Generic direct-call and UFC-call deduplication for the same function
  instantiation is now pinned in typed, HIR, and MIR JSON by
  `emit_json_typed_generic_ufc_dedup_schema_matches_golden`,
...
- Direct Sync effect mode spelling now has a pinned CLI diagnostics JSON gate
...
- Dynamic `String` nested inside a generic type argument now has a pinned CLI
  diagnostics JSON gate, proving the public path preserves the allocator-backed
  text diagnostic through generic annotation validation. Covered by
...
- Result propagation through `.raise()` now has a pinned CLI diagnostics JSON
...
- Checked MIR JSON for generic `Self -> Self` method specialization is now
  pinned by `emit_json_mir_generic_self_method_schema_matches_golden`, covering
  those concrete copy methods returning `self`.
...
- Checked typed JSON for generic `Result<T, E>` enum specialization is now
...
- Diagnostics JSON for duplicate explicit behavior implementations is now
  pinned by `emit_json_diagnostics_duplicate_generic_impl_schema_matches_golden`,
  covering the stable `E3500` payload for duplicate
...
- Diagnostics JSON for generic function call arity is now pinned by
  ...
  without inference or argument-mismatch followups.
- Diagnostics JSON for non-generic function call type arguments is now pinned
  by `emit_json_diagnostics_nongeneric_function_type_args_schema_matches_golden`,
  covering the stable `E5002` payload for `id<i32>(1)` without argument-mismatch
  followups.
- Diagnostics JSON for non-generic module function call type arguments is now
  pinned by
...
- Diagnostics JSON for generic method type-argument annotation arity is now
  ...
  followups.
- Diagnostics JSON for non-generic method call type arguments is now pinned by
...
- Diagnostics JSON for missing generic method type-argument annotation
  ...
  argument-mismatch followups.
- Diagnostics JSON for closure parameter generic annotation arity is now pinned
  by
...
- Diagnostics JSON for cast target generic annotation arity is now pinned by
  `emit_json_diagnostics_cast_target_annotation_type_arg_arity_schema_matches_golden`,
  covering the stable `E5001` payload for
  `cast(box, Box<i32, StaticString>)`.
- Diagnostics JSON for cast target generic annotation missing arguments is now
  pinned by
  `emit_json_diagnostics_cast_target_annotation_missing_args_schema_matches_golden`,
  covering the stable `E5001` payload for `cast(box, Box)`.
- Diagnostics JSON for nested generic annotation inner arity is now pinned by
...
- Diagnostics JSON for function type parameter generic annotation arity is now
  pinned by
  `emit_json_diagnostics_function_type_parameter_annotation_arity_schema_matches_golden`,
  covering the stable `E5001` payload for `(Box<i32, StaticString>) i32`.
- Diagnostics JSON for function type return generic annotation missing
  arguments is now pinned by
  `emit_json_diagnostics_function_type_return_annotation_missing_args_schema_matches_golden`,
  covering the stable `E5001` payload for `() Box` without return-mismatch
  followups.
- Diagnostics JSON for pointer inner generic annotation arity is now pinned by
  `emit_json_diagnostics_pointer_inner_generic_annotation_arity_schema_matches_golden`,
  covering the stable `E5001` payload for `Ptr<Box<i32, StaticString>>`.
- Diagnostics JSON for slice inner generic annotation missing arguments is now
  pinned by
...
- Diagnostics JSON for non-generic struct constructor type arguments is now
  pinned by
  `emit_json_diagnostics_nongeneric_struct_constructor_type_args_schema_matches_golden`,
  covering the stable `E5002` payload for `Point<i32> { x: 1 }` without
  field-mismatch followups.
...
- Diagnostics JSON for missing generic enum constructor type arguments is now
  pinned by
  `emit_json_diagnostics_generic_enum_constructor_missing_args_schema_matches_golden`,
  covering the stable `E5001` payload for `Option.Some(1)` without
  payload-mismatch followups.
- Diagnostics JSON for non-generic enum constructor type arguments is now
  pinned by
  ...
  payload-mismatch followups.
- Diagnostics JSON for generic struct annotation arity is now pinned by
...
- Diagnostics JSON for non-generic enum annotation type arguments is now
  pinned by
  `emit_json_diagnostics_nongeneric_enum_annotation_type_args_schema_matches_golden`,
  covering the stable `E5002` payload for `Direction<i32>` in a function
  parameter type without dependent-use followups.
- Diagnostics JSON for generic enum annotation arity is now pinned by
  ...
  function parameter type without dependent-use followups.
- Diagnostics JSON for missing generic struct annotation type arguments is now
  pinned by
  `emit_json_diagnostics_generic_struct_annotation_missing_args_schema_matches_golden`,
  covering the stable `E5001` payload for bare `Box` in a function parameter
  type without dependent-use followups.
- Diagnostics JSON for missing generic enum annotation type arguments is now
  pinned by
  `emit_json_diagnostics_generic_enum_annotation_missing_args_schema_matches_golden`,
  covering the stable `E5001` payload for bare `Option` in a function parameter
  type without dependent-use followups.
...
- Legacy `build-graph build.zen` execution tests now keep single-target,
...
- Imported duplicate generic behavior impls are rejected through the module
  graph, covered by
  `integration::imported_duplicate_generic_behavior_impl_is_error`, with public
  JSON shape pinned by
  `emit_json_diagnostics_duplicate_generic_impl_schema_matches_golden`.
...
- Generic method-call checking now shares one specialization path for direct
...
- Generic direct-call and UFC-call deduplication for the same function
  ...
  definition, plus typed/HIR/MIR JSON golden tests proving frontend JSON views
...
- Generic worklist deduplication is now covered by
  ...
  shared concrete `inner_i32` definition is emitted once, and typed/HIR/MIR
  JSON golden tests proving `left_i32` and `right_i32` both reuse it.
...
- Legacy `zen build-graph build.zen` validation now keeps execution-scope,
  dependency-shape, target-metadata, gated-dependency, and graph-only-library
...
- `emit-json build-graph <build.zen>` now has matching graph-emission
  diagnostics for malformed `Library` target metadata, covered by
...
- All executing/checking `build.zen` command entrypoints now reject malformed
  ...
  and invalid root field types before outputs are created, covered by
...
- All executing/checking `build.zen` command entrypoints now reject ordinary
  unknown `Test` and `Library` target fields before outputs are created,
  covered by `build_zen_commands_reject_unknown_test_target_fields` and
...
- The Required Test Backlog now scopes behavior-association remaining work to
  generated/fallback association, since explicit `Json<T>` impl/requires/bound
...
- Symbols JSON for generic `Result<T, E>` enum templates is now pinned by
  `emit_json_symbols_generic_result_schema_matches_golden`, covering
...
- Public docs now also avoid implying that interpolation is baked storage:
  interpolation is documented as a `StaticString`-shaped non-owning view that
...
- Actor framework type spellings `Actor`, `ActorRef`, `Mailbox`, and
  ...
  `semantic_builtin_type_checks_use_shared_spelling_helper`.
- Bare actor framework type spellings `Actor`, `ActorRef`, and `Mailbox` now
...
- Infix `expr as Type` cast syntax has been removed in favor of the documented
  prefix `cast(value, Type)` form. Guarded by
...
- HIR JSON for generic `Option<T>` specialization is now pinned by
...
- HIR JSON for nested generic `Result<Option<T>, E>` specialization is now
  ...
  function signatures.
- HIR JSON for explicit generic behavior association dispatch is now pinned by
...
- The checked minimal MIR function/block/terminator schema is now pinned by a
  golden JSON fixture, guarded by
...
- Hand-authored AST JSON inputs to `emit-json ast` now reject at the
  compiler-owned AST JSON boundary before forged unchecked syntax trees can be
  treated as Zen source or accepted as AST evidence. Guarded by
...
- Atomic intrinsic gate diagnostics are now pinned at the CLI JSON boundary by
  `emit_json_diagnostics_atomic_gate_schema_matches_golden`, covering

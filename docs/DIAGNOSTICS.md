...
| `E0203` | gated compiler-owned intrinsic call, including reserved async scheduler intrinsics, atomic intrinsics, raw syscalls, raw allocation intrinsics, byte-memory intrinsics, and raw pointer intrinsics before Sync/Async task lowering, memory-order rules, host-effect declarations, syscall ABI semantics, allocator ownership, pointer provenance, and layout semantics are implemented. | `tests/fixtures/ir_json/diagnostics_async_intrinsic_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_atomic_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_syscall_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_raw_allocate_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_byte_memory_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_raw_pointer_gate.golden.json` |
| `E3500` | resolver validation failure, including duplicate explicit type association requirements/implementations and gated reserved type surfaces. | `tests/fixtures/ir_json/diagnostics_duplicate_generic_requires.golden.json`, `tests/fixtures/ir_json/diagnostics_duplicate_generic_impl.golden.json`, `tests/fixtures/ir_json/diagnostics_typed_allocator_effect_gate.golden.json` |

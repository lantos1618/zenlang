...
| `E0203` | gated compiler-owned intrinsic call, including reserved async scheduler intrinsics, raw allocation intrinsics, and byte-memory intrinsics before Sync/Async task lowering and allocator ownership are implemented. | `tests/fixtures/ir_json/diagnostics_async_intrinsic_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_raw_allocate_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_byte_memory_gate.golden.json` |

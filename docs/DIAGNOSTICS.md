...
| `E0203` | gated compiler-owned intrinsic call, including reserved async scheduler intrinsics, raw allocation intrinsics, byte-memory intrinsics, and raw pointer intrinsics before Sync/Async task lowering, allocator ownership, pointer provenance, and layout semantics are implemented. | `tests/fixtures/ir_json/diagnostics_async_intrinsic_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_raw_allocate_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_byte_memory_gate.golden.json`, `tests/fixtures/ir_json/diagnostics_raw_pointer_gate.golden.json` |

...
- Gated primitive surfaces remain explicit: `atomic_intrinsics_are_rejected_as_effect_gates`, `comptime_type_match_intrinsic_is_rejected_as_gated_not_unknown`, `raw_memory_intrinsics_are_rejected_as_allocator_gates`, and `syscall_intrinsics_are_rejected_as_host_effect_gates`. Allocator-backed `String` is stdlib-owned Zen surface; without a definition or import it follows the ordinary unknown-type resolver path.
- Largest unparseable stdlib sketches have been compacted to parse-only placeholders. Broad stdlib APIs remain unpromoted until parser, typechecker, build, and docs-truth evidence proves them through the same path as user modules.

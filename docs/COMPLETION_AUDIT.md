...
- Diagnostics JSON now carries structured `suggested_fixes` for the removed
...
- Check-command build graph host-effect integration tests now split env-read
  ...
  `tests/integration/cli_build/graph_validation_host_effects/env_reads/`,
...
- `emit-json build-graph` rejects unsupported package/link targets through the
  ...
  It also rejects recognized-but-gated package/link fields through
  ...
  The same execution/check entrypoints now reject gated package/link fields
...
- `build-graph <build.zen>` consumes the deterministic graph for executable
  ...
  `cargo test --test integration build_graph_command_accepts_wildcard_fallback_declared_env_read_for_multiple_targets`,
  and
  `cargo test --test integration build_graph_command_accepts_identifier_fallback_declared_env_read_for_multiple_targets`.
...
- `zen emit build.zen` now has executable graph fixtures for `.Err`,
  ...
  undeclared file-read rejection before C emission. Coverage:
...
- All executing/checking `build.zen` command entrypoints now reject malformed
  `Library` target metadata for duplicate fields, missing required `exports`,
  invalid `exports` field types, and empty export lists before creating
  outputs. Coverage: `build_zen_commands_reject_duplicate_library_target_fields`,
...
- `emit-json build-graph <build.zen>` now also rejects ordinary unknown
  `Test` and `Library` target fields before graph emission. Coverage:
...
- All executing/checking `build.zen` command entrypoints now reject malformed
  ...
  and invalid field value types before creating outputs. Coverage:
...
- Hand-authored AST JSON inputs to `emit-json ast` reject at the compiler-owned
  AST JSON boundary before forged unchecked syntax trees can be treated as Zen
  source or accepted as AST evidence. Covered by

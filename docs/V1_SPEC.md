...
`Type.implements(Json<T>)`, comptime execution, type matching, async operations,
...
| Type association assertions `.requires` | experimental | `parser::tests::parse_behavior_requires_assertion`, `parser::tests::parse_rejects_generic_behavior_requires_with_clear_error`, `resolver_phase2::resolver_accepts_behavior_requires_known_type_and_behavior`, `typechecker::tests::behavior_requires_rejects_missing_impl` |

...
- `JSON/YAML IR boundaries`: gated. JSON is the machine-readable exchange format
  ...
  `Json<StaticString>` impl/require metadata on `Point`.
  ...
  typed call sites. Nested generic typed JSON is pinned by
  ...
  `unwrap_or_i32` and `unwrap_or_i32_StaticString` function signatures. Nested
  ...
  match-arm lowering. Nested generic MIR specialization is pinned by

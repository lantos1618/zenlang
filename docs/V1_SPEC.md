...
| Generic specialization for functions, structs, enums, and methods | implemented | `generic_specializations_emit_each_generated_c_definition_once`, `compile_to_c_with_generated_call_check`, `undefined_generated_c_calls`, `generic_specializations::enum_generated_c::enum_specializations_do_not_emit_unspecialized_c_symbols`, `tests/zen/generic_method_worklist.zen` |
...
- `StaticString` is baked into the program. It denotes literal/static text with stable storage and compile-time length in the generated runtime layout. String literals as baked `StaticString`; interpolation as non-owning `StaticString` views. Allocator-backed `String` is owned dynamic text with allocator identity before promotion, but that type is Zen stdlib source surface, not a Rust-reserved builtin. String literals do not implicitly allocate or coerce into `String`; dynamic construction must use an explicit allocator-aware path. Until a module defines or imports `String`, source-level `String` follows the ordinary unknown-type resolver path.
- `Sync/Async effects`: gated at the semantic boundary, but active-framework APIs are stdlib-owned Zen source. Sync code must not call async operations except through an explicit runtime blocking boundary. Async operations lower through Zen stdlib task, queue, scheduler, yield, and await-like APIs; Rust should not own active-framework import policy or scheduler primitive names. Raw compiler hooks such as `@builtin.atomic_load` are guarded by one generic `@builtin.<name>` gate until the Zen stdlib compiler facade defines them.
- `Typed allocators`: gated at the raw primitive boundary. `Allocator<T, Sync>` and `Allocator<T, Async>` are distinct typed allocator modes in Zen stdlib source, not Rust builtin type names. Raw allocation and byte-memory hooks such as `@builtin.raw_allocate` and `@builtin.memcpy` are guarded by one generic `@builtin.<name>` gate until the Zen stdlib compiler facade defines them.
...
- `Actors in std`: experimental Zen stdlib source; active-framework implementation policy belongs in `stdlib/concurrency`, not Rust import routing or Rust builtin type gates. `Channel` remains an experimental stdlib channel sketch, not a global actor builtin.
- `Ownership and raw pointer operations`: gated by the generic `@builtin.<name>` compiler-facade boundary. Raw pointer operations such as `@builtin.gep`, `@builtin.raw_ptr_cast`, `@builtin.ptr_to_int`, `@builtin.int_to_ptr`, `@builtin.load<T>`, and `@builtin.store<T>` are not Rust-owned user APIs.
- `Host syscalls`: gated at the raw primitive boundary. Higher-level syscall and io_uring facades live in Zen stdlib source; raw hooks such as `@builtin.syscall0` and `@builtin.syscall6` are guarded by the generic `@builtin.<name>` compiler-facade boundary.
...
| Existing broad stdlib files | parse-only placeholders / experimental | Must parse, typecheck, build, and gain docs-truth evidence before promotion |
...
## Stdlib Gate
Files under `stdlib/` are experimental unless a test proves they parse, typecheck, and build through the same compiler path as user modules. Aspirational stdlib APIs must not be described as implemented until promoted by tests; large unparseable sketches should stay as parse-only placeholders rather than false implementation evidence.

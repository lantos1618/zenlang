...
exist. This includes generic behavior bounds such as `T: Json<T>`, generic
behavior inheritance such as `PrettyJson.extends(Json<T>)`, unspecialized
generic behavior bounds such as `T: Json`, unspecialized generic type
association targets such as `Box.implements(Json)`, comptime execution, type

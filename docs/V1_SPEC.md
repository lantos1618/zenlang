...
- `JSON/YAML IR boundaries`: gated. JSON is the machine-readable exchange format
  ...
  worklist specialization emits concrete calls. Symbols JSON for generic
  UFC-only function templates is pinned by
  `emit_json_symbols_generic_ufc_function_schema_matches_golden`, covering
  resolver-owned `id<T>` metadata before `12.id<i32>()` lowers to the concrete
  `id_i32` checked specialization. Symbols JSON for generic Option enum templates
  ...
  JSON for generic `Box<T>.impl` method specialization is pinned by
  `emit_json_typed_generic_type_impl_methods_schema_matches_golden`, covering
  the concrete `Box_i32` type plus `Box.replace_i32` and `Box.get_i32` typed
  call/output paths. Checked typed
  JSON for generic `Self -> Self` method specialization is pinned by
  `emit_json_typed_generic_self_method_schema_matches_golden`, covering
  `Box.copy_i32`, `Box.copy_Option_i32`, and `Option.copy_i32` typed call/output
  paths. Checked typed
  ...
  JSON for generic `Vec<T>` struct/method specialization is pinned by
  `emit_json_typed_generic_vec_schema_matches_golden`, covering concrete
  `Vec_i32` and `Vec_StaticString` payloads plus specialized `Vec.len_i32` and
  `Vec.len_StaticString` call sites. Checked typed
  ...
  typed JSON for generic `Option<T>` multi-specialization is pinned by
  `emit_json_typed_generic_option_multi_schema_matches_golden`, covering
  concrete `Option_i32` and `Option_bool` enum payloads plus specialized
  `Option.unwrap_or_i32` and `Option.unwrap_or_bool` call sites. Checked
  ...
  typed call sites. Checked typed JSON for generic `Result<T, E>`
  multi-specialization is pinned by
  `emit_json_typed_generic_result_multi_schema_matches_golden`, covering
  concrete `Result_i32_StaticString` and `Result_bool_StaticString` enum
  payloads plus specialized `Result.unwrap_or_i32_StaticString` and
  `Result.unwrap_or_bool_StaticString` call sites. Checked typed JSON for
  generic `Result<T, E>` enum method
  ...
  match typing. Checked typed JSON for generic methods returning nested generic
  enums is pinned by
  `emit_json_typed_generic_method_nested_result_schema_matches_golden`,
  covering `Box.wrap_result_i32`, `Result_Option_i32_StaticString`,
  dependent `Option_i32`, and the specialized unwrap call sites that consume
  the method result. Nested generic typed JSON is pinned by
  ...
  `Box_i32` and `Box.get_i32` declarations. Generic `Self -> Self` method HIR
  output is pinned by
  `emit_json_hir_generic_self_method_schema_matches_golden`, covering
  `Box.copy_i32`, `Box.copy_Option_i32`, and `Option.copy_i32`. Generic
  ...
  `Box.get_i32` lowering to `self.value`. Generic `Self -> Self` method MIR
  output is pinned by
  `emit_json_mir_generic_self_method_schema_matches_golden`, covering
  `Box.copy_i32`, `Box.copy_Option_i32`, and `Option.copy_i32` returning
  `self`. Generic method worklist MIR output is pinned by

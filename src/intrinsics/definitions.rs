...
pub(super) fn build_intrinsics() -> HashMap<&'static str, Intrinsic> {
    let mut m = HashMap::new();

    let ptr = AstType::RawPtr(Box::new(AstType::U8));
    let ptr64 = AstType::RawPtr(Box::new(AstType::U64));
    // Overflow-checked arithmetic returns a stdlib `OverflowResult { result, overflow }`.
    let overflow_result = AstType::Named("OverflowResult".to_string());
    ...
    // -- Inline C ---------------------------------------------------------
    intrinsic!(m, "inline_c", ("code" => AstType::Str) -> AstType::Void, "Inline C code compilation", "FFI");
    ...
    intrinsic!(m, "panic",       ("message" => AstType::Str) -> AstType::Void, "Trigger a panic with message", "Debug");
    ...
    // -- FFI/dynamic loading ----------------------------------------------
    intrinsic!(m, "load_library",   ("path" => AstType::Str) -> ptr.clone(), "Load a dynamic library", "FFI");
    intrinsic!(m, "get_symbol",     ("lib_handle" => ptr.clone(), "symbol_name" => AstType::Str) -> ptr.clone(), "Get symbol from library", "FFI");
    ...
    // -- String operations ------------------------------------------------
    intrinsic!(m, "strlen", ("value" => AstType::Str) -> AstType::Usize, "Get length of a static string", "String");
    intrinsic!(m, "static_string_ptr", ("value" => AstType::Str) -> ptr.clone(), "Get pointer to static string data", "String");

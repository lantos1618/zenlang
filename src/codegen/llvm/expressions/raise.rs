//! Raise expression compilation for Result type error propagation.
//!
//! The `.raise()` method on Result types provides early return on error,
//! similar to Rust's `?` operator.

use super::super::LLVMCompiler;
use super::utils::{ast_type_to_basic_type, generic_enum_struct_type, track_result_types};
...
pub fn compile_raise_expression<'ctx>(
    ...
    let raise_id = compiler.raise_counter;
    compiler.raise_counter += 1;

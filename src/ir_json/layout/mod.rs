...
impl<'a> LayoutContext<'a> {
            ...
            TypeDefKind::Enum { variants } => {
                ...
                for variant in variants {
                    let payload_fields = variant
                        ...
                        .iter()
                        .map(|payload| {
                            let field_layout = self.layout_type(payload);
                            max_payload_size = max_payload_size.max(field_layout.size);
                            max_payload_align = max_payload_align.max(field_layout.alignment);
                            LayoutJsonField {
                                name: "payload".to_string(),
                                r#type: payload.display_name(),
                                offset: 0,
                            }
                        })
                        .collect();
    ...
    fn layout_type(&mut self, ty: &Type) -> LayoutJsonType {
        ...
        match ty {
            Type::Named(name) | Type::Enum { name, .. } => self.layout_named(name),

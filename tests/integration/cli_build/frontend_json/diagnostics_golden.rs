...
fn emit_json_diagnostics_typed_allocator_effect_gate_schema_matches_golden() {
    let tmp = tempfile::tempdir().expect("create temp dir");
    let zen_path = tmp.path().join("typed_allocator_effect_gate.zen");
    std::fs::write(
        &zen_path,
        r#"
...
"#,
    ...
    .expect("write typed allocator effect gate source");
    ...
    assert!(
        !output.status.success(),
        "zen emit-json diagnostics should fail on typed allocator effect gate: stdout={}, stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    assert_eq!(
        json["diagnostics"]
            ...
            .len(),
        1,
        "typed allocator/effect gate should emit one feature-gate diagnostic: {json}"
    );

    let normalized = actual.replace(tmp.path().to_str().expect("tmp path is UTF-8"), "$TMP");
    let expected_path =
        fixture("tests/fixtures/ir_json/diagnostics_typed_allocator_effect_gate.golden.json");
    let expected = std::fs::read_to_string(&expected_path)
        .unwrap_or_else(|err| panic!("read {}: {err}", expected_path.display()));
...
fn emit_json_diagnostics_async_intrinsic_gate_schema_matches_golden() {
    let tmp = tempfile::tempdir().expect("create temp dir");
    let zen_path = tmp.path().join("async_intrinsic_gate.zen");
    std::fs::write(
        &zen_path,
        r#"
main = () void {
...
"#,
    ...
    .expect("write async intrinsic gate source");
    ...
    assert!(
        !output.status.success(),
        "zen emit-json diagnostics should fail on async intrinsic gate: stdout={}, stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    assert_eq!(
        json["diagnostics"]
            ...
            .len(),
        1,
        "async intrinsic gate should emit one gated intrinsic diagnostic: {json}"
    );

    let normalized = actual.replace(tmp.path().to_str().expect("tmp path is UTF-8"), "$TMP");
    let expected_path =
        fixture("tests/fixtures/ir_json/diagnostics_async_intrinsic_gate.golden.json");
    let expected = std::fs::read_to_string(&expected_path)
        .unwrap_or_else(|err| panic!("read {}: {err}", expected_path.display()));

...
fn emit_json_diagnostics_generic_struct_constructor_missing_args_schema_matches_golden() {
    ...
    let zen_path = tmp
        .path()
        .join("generic_struct_constructor_missing_args.zen");
    std::fs::write(
        &zen_path,
        r#"
Box<T>: {
...
"#,
    ...
    .expect("write generic struct constructor missing args source");
    ...
    assert!(
        !output.status.success(),
        "zen emit-json diagnostics should fail on generic struct constructor missing args: stdout={}, stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    assert_eq!(
        json["diagnostics"]
            ...
            .len(),
        1,
        "generic struct constructor missing-args diagnostics should not emit field mismatch followups: {json}"
    );
...
fn emit_json_diagnostics_generic_enum_constructor_missing_args_schema_matches_golden() {
    let tmp = tempfile::tempdir().expect("create temp dir");
    let zen_path = tmp.path().join("generic_enum_constructor_missing_args.zen");
    std::fs::write(
        &zen_path,
        r#"
Option<T>:
...
"#,
    ...
    .expect("write generic enum constructor missing args source");
    ...
    assert!(
        !output.status.success(),
        "zen emit-json diagnostics should fail on generic enum constructor missing args: stdout={}, stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    assert_eq!(
        json["diagnostics"]
            ...
            .len(),
        1,
        "generic enum constructor missing-args diagnostics should not emit payload mismatch followups: {json}"
    );
...
fn emit_json_diagnostics_generic_enum_annotation_missing_args_schema_matches_golden() {
    let tmp = tempfile::tempdir().expect("create temp dir");
    let zen_path = tmp.path().join("generic_enum_annotation_missing_args.zen");
    std::fs::write(
        &zen_path,
        r#"
Option<T>:
...
"#,
    ...
    .expect("write generic enum annotation missing args source");
    ...
    assert!(
        !output.status.success(),
        "zen emit-json diagnostics should fail on generic enum annotation missing args: stdout={}, stderr={}",
        String::from_utf8_lossy(&output.stdout),
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    assert_eq!(
        json["diagnostics"]
            ...
            .len(),
        1,
        "generic enum annotation missing-args diagnostics should not emit dependent-use followups: {json}"
    );

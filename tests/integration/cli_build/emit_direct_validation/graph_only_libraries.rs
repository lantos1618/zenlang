use super::super::support::{
    assert_emit_c_source, assert_no_build_dir, assert_zen_failure_contains, assert_zen_success,
    run_zen_in, write_graph_only_library_project, GraphOnlyLibrarySource,
...
fn emit_command_build_zen_rejects_graph_only_library_type_errors() {
    ...
    assert_no_build_dir(
        tmp.path(),
        "zen emit build.zen after graph-only library typechecking failure",
    );
}

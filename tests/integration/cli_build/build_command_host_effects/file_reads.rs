...
#[test]
fn build_command_multi_target_build_zen_rejects_file_read_without_fallback_before_execution() {
    let tmp = tempfile::tempdir().expect("create temp dir");
    std::fs::write(
        tmp.path().join("build.zen"),
        r#"
...
"#,
    ...
    assert!(
        !tmp.path().join("build").exists(),
        "multi-target build should not start after graph validation fails"
    );
}

#[test]
fn build_command_multi_target_build_zen_rejects_undeclared_file_read_effects() {
    let tmp = tempfile::tempdir().expect("create temp dir");
    std::fs::write(
        tmp.path().join("build.zen"),
        r#"

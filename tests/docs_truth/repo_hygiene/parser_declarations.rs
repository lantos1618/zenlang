...
#[test]
fn parser_example_type_tests_share_single_declaration_helper() {
    let example_type_tests = read("src/parser/tests/examples_types_errors.rs");
    let parser_tests = read("src/parser/tests.rs");
    ...
    assert!(
        !example_type_tests.contains("assert_eq!(prog.declarations.len(), 1)"),
        "parser example/type tests should not repeat trivial one-declaration assertions"
    );
}

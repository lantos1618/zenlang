...
fn parsed_project_build_zen_lowers_to_executable_and_test_graph() {
    ...
    assert_eq!(target.name.as_str(), "myapp");
    assert_eq!(target.sources.as_slice(), ["main.zen"]);
    match &target.kind {
    ...
    assert_eq!(target.name.as_str(), "test");
    assert_eq!(target.sources.as_slice(), ["test.zen"]);
    match &target.kind {
